          ...
          <div className="w-full max-w-2xl rounded-lg bg-white shadow-xl">
            ...
            <form onSubmit={submit} className="grid gap-4 px-6 py-5">
              ...
              <label className="grid gap-2 text-sm text-slate-700">
                ...
                <input value={form.password} onChange={(event) => setForm({ ...form, password: event.target.value })} required={!selected} minLength={form.password || !selected ? 8 : undefined} placeholder={selected ? 'Leave blank to keep current password' : 'At least 8 characters'} className="rounded-lg border border-slate-300 bg-slate-50 px-4 py-3 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100" />
              </label>
